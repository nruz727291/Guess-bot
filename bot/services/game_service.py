...
Manages game state, turns, guesses, timeouts, and rewards.
"""
...
# Track turn timeout tasks so we can cancel them
# {room_code: asyncio.Task}
_turn_timers: dict[str, asyncio.Task] = {}

# Track current guess range per room for better UX
# {room_code: {"low": int, "high": int}}
...
async def start_game(client: Client, room_code: str) -> Tuple[bool, str]:
    ...
    start_msg = (
        ...
        f"Room: `{room_code}`\n"
        f"Players:\n{player_list}\n\n"
        f"🔢 I'm thinking of a number between **{Config.NUMBER_MIN}** and **{Config.NUMBER_MAX}**\n\n"
...
async def start_turn(client: Client, room_code: str):
    ...
    # Get current range
    rng = _guess_ranges.get(room_code, {"low": 1, "high": 100})
    
    # Build turn message
    turn_msg = (
        f"🎯 **{player_name}'s Turn!**\n\n"
        f"📊 Range: **{rng['low']}** – **{rng['high']}**\n"
        f"⏱ {Config.TURN_TIMEOUT}s to guess!"
    )
    
    # Broadcast to everyone
    all_players = game["players"]
    
    # Give everyone the status update
    for p in all_players:
        try:
            if p["user_id"] == current_player_id:
                # Active player gets the guess keyboard
...
async def process_guess(client: Client, room_code: str, user_id: int, guess: int) -> Optional[str]:
    ...
    Returns: error message string, or None if guess was processed successfully
    """
    game = await game_db.get_game(room_code)
    if not game or game["state"] != GameState.ACTIVE:
    ...
    # Check guess cooldown (prevent spam clicking)
...
async def _turn_timeout(client: Client, room_code: str, player_id: int):
    """
    Background task: called when a player's turn timer expires.
    Eliminates the inactive player.
    """
...
def _cancel_turn_timer(room_code: str):
    """Cancel a running turn timer if it exists."""
    task = _turn_timers.pop(room_code, None)
    if task and not task.done():
        task.cancel()


async def broadcast_to_players(client: Client, players: list, message: str):
    """Send a message to all players in a game. Silently ignore failures."""
    for player in players:
        try:
